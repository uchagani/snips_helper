...
class Bundle:
    ...
    def intent_exists(self, intent_name):
        ...
        for intent in existing_intents:
            if intent.name == intent_name:
                self.logger.debug("Intent exists: {}".format(intent_name))
                return True

        return False

    def create_intent(self, intent_name, utterances=None, slots=None):
        self.logger.debug("Creating new intent: {}".format(intent_name))

        if self.intent_exists(intent_name):
            return

        self.__get_create_intent_button().click()
        intent = Intent(intent_name, self.driver)
        intent.create(utterances, slots)
        return intent
    ...
    #     return utterance_section.find_element_by_class_name(
    # 'dots-icon-button')
